...
def devide_data(input_path, save_bpm_path, save_png_path, create_png, percent, progressbar):
    ...
    for folder in data:
        ...
        for img in images[:math.ceil(len(images) * percent / 100)]:
            ...
            save_path = save_bpm_path + "\\" + TEST_FOLDER + "\\" + splitted[len(splitted) - 2] + "\\" + splitted[len(splitted) - 1]
            im.save(save_path,"bmp")

            if create_png == True:
                save_path = save_png_path + "\\" + TEST_FOLDER + "\\" + splitted[len(splitted) - 2] + "\\" + splitted[len(splitted) - 1]
        ...
        for img in images[math.ceil(len(images) * percent / 100):]:
            im = Image.open(img)
            splitted = str(img).split("\\")
            save_path = save_bpm_path + "\\" + TRAIN_FOLDER + "\\" + splitted[len(splitted) - 2] + "\\" + splitted[len(splitted) - 1]
            im.save(save_path,"bmp")

            if create_png == True:
                save_path = save_png_path + "\\" + TRAIN_FOLDER + "\\" + splitted[len(splitted) - 2] + "\\" + splitted[len(splitted) - 1]
                im.save(save_path, "png")
            completed += step
            progressbar.setValue(completed)
